...
/**
 * Calculate the amount of base and fyToken a user would trade to arrive at a lower tick. 
 * @param { BigNumber | string } baseReserves
 * @param { BigNumber | string } fyTokenReserves
 * @param { BigNumber | string } tick
 * @param { BigNumber | string } timeTillMaturity
 * @param { number } decimals
 * @param { boolean } withNoFee
 * @returns { BigNumber, BigNumber } x_end, y_end
 */
export function tradeToTick(
  baseReserves: BigNumber | string,
  fyTokenReserves: BigNumber | string,
  tick: BigNumber | string,
  timeTillMaturity: BigNumber | string,
  decimals: number, // optional : default === 18
  withNoFee: boolean = false // optional: default === false
): [BigNumber, BigNumber] {
  ...
  const tick18 = decimalNToDecimal18(BigNumber.from(tick), decimals);
  const baseReserves_ = new Decimal(baseReserves18.toString());
  const fyTokenReserves_ = new Decimal(fyTokenReserves18.toString());
  
  const wad_ = new Decimal(WAD_BN.toString());
  const tick_ = (new Decimal(tick18.toString())).div(wad_);
  const ratio = fyTokenReserves_.div(baseReserves_);
  //console.log("tick", tick_, "ratio", ratio);
  const _g = ratio > tick_ ? g1 : g2;
  //console.log("g is ", _g)
  const [a, invA] = _computeA(timeTillMaturity, _g);
  ...
  const sum = ONE_DEC.add(tick_.pow(a));
  const x_end_ = ZYsum.div(sum).pow(invA);
  const y_out = toBn(x_end_.mul(tick_).sub(fyTokenReserves_));
  const x_out = toBn(x_end_.sub(baseReserves_)); 
...
/**
 * Calculate the tick ratio from the tick number. 
 ...
 */
/*
export function tickNumberToRatio(
  tickNumber : number,
  fyTokenReserves: BigNumber | string,
  tick: BigNumber | string,
  timeTillMaturity: BigNumber | string,
 ...
 * @param { BigNumber | string } tradeValue // current [base]
 * @param { BigNumber | string } amount // y[base] amount at maturity
 * @param { number } timeTillMaturity  // seconds to maturity
 * @returns { string | undefined } human readable string
 */
export const getAPR = (
  tradeValue: BigNumber | string,
  ...
  const propOfYear = new Decimal(timeTillMaturity / SECONDS_PER_YEAR);
  const priceRatio = amount_.div(tradeValue_);
  const powRatio = ONE.div(propOfYear);
  const apr = priceRatio.pow(powRatio).sub(ONE);

  console.log("propOfYear", propOfYear.toString())
  console.log("priceRatio", priceRatio.toString())
  console.log("powRatio", powRatio.toString())
...
/**
 * Calculate APR at desired pool ratio for given time stretch, by calculating a trade.
 * @param { Decimal } ts // timestretch
 * @param { number } timeTillMaturity  // seconds to maturity
 * @returns { string | undefined } human readable string
 */
export const tsToRate = (
  ts : Decimal,
  timeTillMaturity: BigNumber | string,
): string | undefined  => {
  ...
  const baseReserves_ = (new Decimal(1000000)).mul(wad_);
  const fyTokenReserves_ = new Decimal(2000000).mul(wad_);
  const fyDai_ = (new Decimal(1)).mul(wad_);
  const time = new Decimal(timeTillMaturity.toString());

  const _g = g1;
  //const ts = new Decimal(1 / secondsInTenYears.toNumber());
  const [a, invA] = _computeA(time.toString(), _g, ts);
...
/**
 * Calculate APR at desired pool ratio for given time stretch, algebraically.
 * @param { Decimal } ts // timestretch
 * @returns { string | undefined } human readable string
 */
export const tsToRate2 = (
  ts : Decimal
): string | undefined  => {
  ...
  const baseReserves_ = (new Decimal(1000000)).mul(wad_);
  const fyTokenReserves_ = new Decimal(2000000).mul(wad_);
  const secondsInOneYear = new Decimal(31557600);

  const u = ts.mul(secondsInOneYear);
  const r = fyTokenReserves_.div(baseReserves_).pow(u).sub(new Decimal(1));
  return r.mul(100).toFixed();
 ...
 * @returns { BigNumber } 
 */
export const tickNumberToRatio = (
  index : number,
  ts : Decimal
): BigNumber  => {
  ...
  // rate = (1 + 0.01 * index)
  const r = (new Decimal(index)).mul(new Decimal(0.01));
  const rate = ONE.add(r);
  
  // result rate**(1/ts)
  const inv_ts = ONE.div(ts.mul(secondsInOneYear));
  const result = rate.pow(inv_ts).mul(wad_);
  return toBn(result);
...
/**
 * Calculate the value of liquidity in base at a tick. 
 * @param { BigNumber | string } baseReserves
 * @param { BigNumber | string } fyTokenReserves
 * @param { BigNumber | string } fyToken
 * @param { BigNumber | string } timeTillMaturity
 ...
 * @returns { BigNumber, BigNumber } x_end, y_end
 */
export function valueAtTick(
  baseReserves: BigNumber | string,
  fyTokenReserves: BigNumber | string,
  value: BigNumber | string,
  valueAbove: BigNumber | string,
  tick: BigNumber | string,
  timeTillMaturity: BigNumber | string,
  decimals: number, // optional : default === 18
  withNoFee: boolean = false // optional: default === false
  ...
  const baseReserves18 = decimalNToDecimal18(BigNumber.from(baseReserves), decimals);
  const fyTokenReserves18 = decimalNToDecimal18(BigNumber.from(fyTokenReserves), decimals);
  const value18 = decimalNToDecimal18(BigNumber.from(value), decimals);
  const valueAbove18 = decimalNToDecimal18(BigNumber.from(valueAbove), decimals);
  const tick18 = decimalNToDecimal18(BigNumber.from(tick), decimals);

  const baseReserves_ = new Decimal(baseReserves18.toString());
  const fyTokenReserves_ = new Decimal(fyTokenReserves18.toString());
  const value_ = new Decimal(value18.toString());
  const valueAbove_ = new Decimal(valueAbove18.toString());
  const wad_ = new Decimal(WAD_BN.toString());
  const tick_ = (new Decimal(tick18.toString())).div(wad_);
  const ratio = fyTokenReserves_.div(baseReserves_);
  ...
  const sum = ONE_DEC.add(tick_.pow(a));
  const val = value_.add(valueAbove_).pow(a);
  //console.log("sum", sum.toString());
  //console.log("val", val.toString());
  const prod = sum.mul(val);
  //console.log("prod", prod.toString());
  
  const sv = ZYsum.div(prod).pow(invA);
  console.log("sv", sv.toString());
  const sv_out = toBn(sv); 
